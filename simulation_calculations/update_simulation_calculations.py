...
def handle_pb(project, failing_accommodations):
    ...
    for s in simulations:
        if needs_calculation(s):
            ...
            if (not old_total_work_price and not old_total_subventions and not old_remaining_costs and not old_subvention_on_TTC) and \
                (old_total_work_price != 0 and old_total_subventions != 0 and old_remaining_costs != 0 and old_subvention_on_TTC != 0):
                ...
                if sum(total_work_prices) == 0:
                    s.subvention_on_TTC = 0
                else:
                    s.subvention_on_TTC = round(
                        (
                            (
                                sum(total_work_prices)
                                - sum(remaining_costs)
                            )
                            / sum(total_work_prices)
                        )
